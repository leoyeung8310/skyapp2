...
function fontHeightV2(height) {
    var minHeight = Math.max(height, MorphicPreferences.minimumFontHeight);
    return minHeight * 1; // assuming 1/5 font size for ascenders
}
...
StringMorph.prototype.drawNew = function () {
    ...
    this.bounds.corner = this.bounds.origin.add(
        new Point(
            width,
            fontHeightV2(this.fontSize) + Math.abs(shadowOffset.y)
        )
    );
    ...
    if (this.shadowColor) {
        ...
        context.fillText(txt, x, fontHeightV2(this.fontSize) + y);
    }
    ...
    if (this.isShowingBlanks) {
        this.renderWithBlanks(context, x, fontHeightV2(this.fontSize) + y);
    } else {
        context.fillText(txt, x, fontHeightV2(this.fontSize) + y);
    }
    ...
    for (i = start; i < stop; i += 1) {
        ...
        context.fillText(c, p.x + x, fontHeightV2(this.fontSize) + y);
    }
...
WorldMorph.prototype.slide = function (aStringOrTextMorph) {
    ...
    slider.silentSetWidth(MorphicPreferences.menuFontSize * 20);
    slider.drawNew();
    slider.action = function (num) {
